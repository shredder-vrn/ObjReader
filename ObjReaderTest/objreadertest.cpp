...
void ObjReaderTest::testParseVertexLine_data()
{
    ...
    QTest::newRow("Empty after v") << "v" << false;
    ...
    QTest::newRow("Uppercase V") << "V 1.0 2.0 3.0" << false; // OBJ: только lowercase 'v'
    ...
    QTest::newRow("With letters in numbers") << "v 1a 2b 3c" << false;
    ...
    QTest::newRow("Multiple decimal points") << "v 1.2.3 2.0 3.0" << false;
    ...
    QTest::newRow("Spaces in numbers") << "v 1 . 0 2.0 3.0" << false;
    QTest::newRow("Number starts with dot") << "v .123 2.0 3.0" << true;
    QTest::newRow("Number ends with dot") << "v 123. 2.0 3.0" << true;
    QTest::newRow("Binary number") << "v 0b1010 2.0 3.0" << false;
    //QTest::newRow("Octal number") << "v 0123 2.0 3.0" << false;
    QTest::newRow("Hex number") << "v 0x1A 2.0 3.0" << false;
    QTest::newRow("Scientific notation with sign") << "v -1e+3 2.0 3.0" << true;
    QTest::newRow("Scientific notation with multiple signs") << "v 1e+-3 2.0 3.0" << false;
    QTest::newRow("Very long line") << "v 1.0 2.0 3.0 asdasdasdasdasdasdasdasdasdasdasdasdasdasd" << false;
    QTest::newRow("Line with many spaces") << "v           1.0          2.0         3.0       " << true;
    QTest::newRow("Line with mixed whitespace") << "v\t1.0 \t 2.0 \v\v 3.0" << true;
    QTest::newRow("Line with comments at start") << "#v 1.0 2.0 3.0" << false;
...
void ObjReaderTest::testParseTexCoordLine_data()
{
    ...
    QTest::newRow("Empty after vt") << "vt" << false;
    ...
    QTest::newRow("With commas") << "vt 0,5 0,5" << false;
    QTest::newRow("Numbers with thousand separators") << "vt 1,000.0 2,000.0" << false;
    ...
    //QTest::newRow("Octal number") << "vt 0123 0.5" << false;
    QTest::newRow("Hex number") << "vt 0x1A 0.5" << false;
    QTest::newRow("Scientific notation with sign") << "vt -1e+3 0.5" << true;
    QTest::newRow("Scientific notation with multiple signs") << "vt 1e+-3 0.5" << false;
    QTest::newRow("Very long line") << "vt 0.5 0.5 asdasdasdasdasdasdasdasdasdasdasdasdasdasd" << false;
    QTest::newRow("Line with many spaces") << "vt           0.5          0.5       " << true;
    QTest::newRow("Line with mixed whitespace") << "vt\t0.5 \t 0.5 \v\v" << true;
...
void ObjReaderTest::testParseNormalLine_data()
{
    ...
    QTest::newRow("Multiple dots in number") << "vn 1.0.0 2.0 3.0" << false;
    ...
    QTest::newRow("Trailing comment") << "vn 0.0 1.0 0.0 #comment here" << true;
    ...
    QTest::newRow("Uppercase VN") << "VN 0.0 1.0 0.0" << false; // OBJ требует 'vn'
    QTest::newRow("With slash like face") << "vn 1/2 2/3 3/4" << false;
    QTest::newRow("With underscores") << "vn 1_0 2_0 3_0" << false;
    QTest::newRow("With letters in numbers") << "vn 1a 2b 3c" << false;
   ...
   // QTest::newRow("Octal number") << "vn 0123 0.0 0.0" << false;
    QTest::newRow("Hex number") << "vn 0x1A 0.0 0.0" << false;
    QTest::newRow("Scientific notation with sign") << "vn -1e+3 0.0 0.0" << true;
    QTest::newRow("Scientific notation with multiple signs") << "vn 1e+-3 0.0 0.0" << false;
...
void ObjReaderTest::testParseFaceLine_data()
{
    ...
    QTest::newRow("Missing vt") << "f 1/ 2/ 3/" << true;
    QTest::newRow("Missing vn") << "f 1// 2// 3//" << true;
    ...
    QTest::newRow("Face with very long line") << "f 1 2 3 asdasdasdasdasdasdasdasdasdasdasdasdasdasd" << false;
...
//        model.faceVertexIndices.append(0);
//        model.faceVertexIndices.append(1);
//        model.faceVertexIndices.append(5); // индекс вне диапазона
//        model.polygonStarts.append(0);
//        model.polygonLengths.append(3);
...
//        QTest::newRow("Valid with texture coords") << model << true;
//    }
...
//        model.faceTexCoordIndices.append(0);
//        model.faceTexCoordIndices.append(1);
//        model.faceTexCoordIndices.append(1); // предполагаем, что текстурных координат меньше
//        model.textureVertices.append(QVector2D(0.0f, 0.0f));
//        model.textureVertices.append(QVector2D(1.0f, 0.0f)); // но только две

//        QTest::newRow("Mismatched texture coords count") << model << false;
//    }
//}
...
//void ObjReaderTest::testParseObjFromFile_data()
//{
//    QTest::addColumn<QString>("filePath");
//    QTest::addColumn<bool>("expectedResult");

//    QTest::newRow("Valid cube.obj") << ":/resources/cube.obj" << true;
//    QTest::newRow("Non-existent file") << "/invalid/path.obj" << false;
//    QTest::newRow("File with invalid face") << ":/resources/invalid_face.obj" << false;
//    QTest::newRow("File with only vertices") << ":/resources/vertices_only.obj" << true;
//    QTest::newRow("File with empty lines") << ":/resources/empty_lines.obj" << true;
//}

//void ObjReaderTest::testParseObjFromFile()
//{
//    QFETCH(QString, filePath);
//    QFETCH(bool, expectedResult);

//    Model model;
//    bool result = parseObj(filePath, model);
//    QCOMPARE(result, expectedResult);
//}
